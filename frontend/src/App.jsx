...
const App = () => {
  ...
  // String state is still useful for the File Load/Save component
  ...
  const updateJsonData = useCallback((newDataPart) => {
    ...
    setJsonData(prevData => {
        ...
        setJsonContent(JSON.stringify(updatedData, null, 2)); // Keep string in sync
        return updatedData;
    });
  }, []);
  ...
  // Gets the current JSON object state (still needed for File Save)
  ...
  const menuComponents = useMemo(() => ({
      File: <FileMenuBox ref={activeMenu === 'File' ? activeMenuBoxRef : null} setJsonContent={updateJsonString} onConfigurationChange={updateJsonData} getCurrentJsonData={getCurrentJsonData} />,
  ...
  return (
    <>
      ...
      <Grid container spacing={2} style={{ padding: '16px' }}>
        ...
        <Grid item xs={4}>
          <DisplayWindow />
          {/* --- MODIFIED: Pass jsonContent string and remove getter --- */}
          <JsonText
             jsonString={JSON.stringify(jsonData, null, 2)} // Pass current stringified data
             schema={schema}
             setActiveMenu={setActiveMenu} // Still needed to close menu
             // getCurrentJsonData prop removed
          />
          {/* --- END MODIFIED --- */}
        </Grid>
       </Grid>
