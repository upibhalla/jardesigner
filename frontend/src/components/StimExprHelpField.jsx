 ...
 */
const StimExprHelpField = React.memo(({ id, label, value, onChange, helptext, fullWidth = true, required, ...props }) => {
    const exprError = useMemo(() => validateStimExpr(String(value ?? '')), [value]);
    const emptyError = required && !String(value ?? '').trim() ? 'Stimulus expression is required' : null;
    const displayError = emptyError || exprError;
